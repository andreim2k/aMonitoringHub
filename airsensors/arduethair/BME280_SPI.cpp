...
BME280_SPI::BME280_SPI(SPISettings spiSettings, int csPin)
    : spiSettings(spiSettings), csPin(csPin), chip_id(0), has_humidity(false) {
  ...
  reset();
  if (!wait_for_ready(I2C_STATUS_CHECK_TIMEOUT_MS)) {
    Serial.println(F("{\"error\":\"BM280 not ready after reset\"}"));
    while (1);
  }
...
uint8_t BME280_SPI::_read_register(uint8_t reg) {
...
void BME280_SPI::_read_registers(uint8_t reg, uint8_t count, uint8_t *buf) {
  SPI.beginTransaction(spiSettings);
  digitalWrite(csPin, LOW);
  delayMicroseconds(10);
...
void BME280_SPI::_write_register(uint8_t reg, uint8_t value) {
  SPI.beginTransaction(spiSettings);
  digitalWrite(csPin, LOW);
...
void BME280_SPI::_read_calibration_data() {
  uint8_t calib[24];
...
void BME280_SPI::_configure_sensor() {
  if (has_humidity) {
    _write_register(0xF2, 0x01);
  }
  _write_register(0xF4, 0x27);
  _write_register(0xF5, 0x20);
}

void BME280_SPI::reset() {
  _write_register(BM280_RESET_REGISTER, BM280_RESET_VALUE);
  delay(3);
}

void BME280_SPI::reconfigure() {
  _configure_sensor();
}

uint8_t BME280_SPI::check_status() {
  return _read_register(BM280_STATUS_REGISTER);
}

bool BME280_SPI::is_ready() {
...
bool BME280_SPI::wait_for_ready(unsigned long timeoutMs) {
...
void BME280_SPI::_read_raw_data(uint32_t &raw_temp, uint32_t &raw_press, uint32_t &raw_hum) {
  if (!wait_for_ready()) {
    Serial.println(F("{\"error\":\"BM280 not ready\"}"));
    return;
  }
...
void BME280_SPI::read_compensated_data(float &temp, float &pressure, float &humidity) {
...
float BME280_SPI::read_temperature() {
  float t, p, h;
  read_compensated_data(t, p, h);
  return t;
}

float BME280_SPI::read_pressure() {
  float t, p, h;
  read_compensated_data(t, p, h);
  return p;
}

float BME280_SPI::read_humidity() {
  float t, p, h;
  read_compensated_data(t, p, h);
  return h;
}

int16_t BME280_SPI::_to_signed(int16_t value, uint8_t bits) {
  if (value & (1 << (bits - 1))) {
    return value - (1 << bits);
  }
  return value;
